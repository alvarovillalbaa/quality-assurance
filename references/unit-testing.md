...
> **Using Vitest (Vite projects)?** See `references/vitest/` for deep Vitest-specific coverage:
> `config.md` (vitest.config.ts, CLI, sharding), `test-api.md` (fixtures, concurrent, parameterized),
> `mocking.md` (vi.fn, vi.mock, timers, dates), `utilities.md` (expect API, snapshots, coverage),
> `advanced.md` (environments, type testing, browser mode).
