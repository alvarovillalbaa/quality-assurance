...
**Load this reference when:** writing or changing tests, adding mocks, reviewing test code, or tempted to add test-only methods to production code.
...
**Following strict TDD prevents most of these anti-patterns.** If you are running into them frequently, the root cause is often that tests were written after the implementation.
...
BEFORE creating mock responses:
  1. Examine the actual API response from docs or real usage
...
Testing is part of implementation, not an optional follow-up step. You cannot claim something is complete without tests. TDD would have prevented this — see `agentic-development` skill, `specs-plans-tests.md`.
...
Consider integration tests with real components — they are often simpler than elaborate mock setups.
