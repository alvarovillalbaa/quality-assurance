...
Creates a new Mastra project using `create-mastra@<tag>` and performs smoke testing of the Mastra Studio in Chrome using browser automation.
...
This workflow requires a browser automation MCP server (e.g., Playwright MCP or Claude-in-Chrome MCP) for the Studio smoke test steps.
...
1. **Parse arguments** from the provided string
2. **Check required parameters**:
   - `--directory` / `-d`: REQUIRED — fail if missing
   - `--name` / `-n`: REQUIRED — fail if missing
   - `--tag` / `-t`: REQUIRED — fail if missing
3. **Apply defaults** for optional parameters:
   - `--pm` / `-p`: Default to `npm`
   - `--llm` / `-l`: Default to `openai`
4. **Validate values**:
...
Wait for installation to complete (1–2 minutes depending on network).
...
**Create `src/mastra/agents/activity-agent.ts`:**
...
**Create `src/mastra/agents/planner-network.ts`:**
...
export const plannerNetwork = new Agent({
  ...
  agents: { weatherAgent, activityAgent }, // makes it a network agent
  memory: new Memory(), // REQUIRED for network agents
});
```

**Update `src/mastra/index.ts`** to register the new agents:
...
**Note**: Network mode requires both the `agents` property (sub-agents) and `memory` in the Agent constructor.
...
1. **Global environment**: Run `echo $<ENV_VAR_NAME>` — if set globally, the project inherits it; skip to Step 4.
2. **Project `.env` file**: If not set globally, check if `.env` exists and contains the key.
3. **Ask user only if needed**: If not available globally or in `.env`, request the key and write it to `.env`.

Only check for the ONE key matching the selected provider.
...
Use the browser automation tools (Playwright MCP or equivalent) to test the Mastra Studio.
...
- [ ] Navigate directly to `/scorers/completeness-scorer` (use URL navigation, not click — client-side routing timing issues)
...
- If the user doesn't specify an LLM provider, default to OpenAI
